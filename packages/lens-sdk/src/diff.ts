import type { DiffColumnWindow, DiffLine, DiffLineWindow, DiffModel, DiffModelMetadata } from "./contracts";

export interface TextDiffModelInput {
  oldText: string;
  newText: string;
  summary?: string;
  language?: string;
...
export function createTextDiffModel(input: TextDiffModelInput): DiffModel {
  const oldLines = splitTextLines(input.oldText);
  const newLines = splitTextLines(input.newText);
  const columnWindow =
    input.metadata?.columnWindow ??
    input.columnWindow ??
    normalizeDiffColumnWindow(input.fields?.columnWindow);
  const lines = diffTextLines(oldLines, newLines).map((line) =>
    columnWindow ? applyDiffColumnWindow(line, columnWindow) : line
  );
...
export function normalizeDiffColumnWindow(value: unknown): DiffColumnWindow | undefined {
  ...
  const columnStart =
    getNonNegativeInteger(record.columnStart ?? record.column_start ?? record.startColumn ?? record.start_column ?? record.start) ??
    0;
  const explicitColumnEnd = getNonNegativeInteger(
    record.columnEnd ?? record.column_end ?? record.endColumn ?? record.end_column ?? record.end
  );
  const limit = getPositiveInteger(record.columnLimit ?? record.column_limit ?? record.limit);
  const textLength = getNonNegativeInteger(
    record.textLength ?? record.text_length ?? record.totalColumns ?? record.total_columns ?? record.totalLength ?? record.total_length
  );
  const columnEnd =
    explicitColumnEnd ??
  ...
  const hasMoreColumns =
    getBooleanValue(record.hasMoreColumns ?? record.has_more_columns) ??
...
export function diffTextLines(oldLines: string[], newLines: string[]): DiffLine[] {
...
export function getTextDiffStats(lines: DiffLine[]): TextDiffStats {
  return lines.reduce<TextDiffStats>(
    (stats, line) => {
      if (line.op === "insert") {
        stats.additions += 1;
...
function summarizeTextDiff(stats: TextDiffStats): string {
...
function applyDiffColumnWindow(line: DiffLine, columnWindow: DiffColumnWindow): DiffLine {
  ...
  const hasMoreColumns =
    columnWindow.hasMoreColumns ?? (columnStart > 0 || columnEnd < textLength);
